...
module.exports = {
    obfuscate: async function(source, settings = DefaultSettings) {
        ...
        let inputFile = path.join(__dirname, 'temp', `Obfuscar-${settings.Uid}.lua`)
        let outputFile = path.join(__dirname, 'temp', `Obfuscado-${settings.Uid}.out`)
        ...
        code = `--[[\n\tWarline Obfuscator V${settings.__VERSION__}\n--]]\n\n` + code

        if (settings.JIT && process.platform === 'win32') {
            console.log(`Running file ('./obf/temp/Obfuscado.lua')...`)
            fs.writeFileSync(`./obf/temp/Obfuscado.lua`, code)
            try {
                await exec(`cd temp && lua Obfuscado.lua`, { timeout: 2000 }, (err, stdout, stderr) => {
                    if (err) {
                        if (err.killed) {
                            console.log("Timeout!")
                        } else {
        ...
        let outPath = path.join(__dirname, 'temp', `Obfuscado-${settings.Uid}.lua`)
